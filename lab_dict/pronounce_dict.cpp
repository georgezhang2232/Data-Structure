...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.count(string1) == 0 || dict.count(string2) == 0) return false;

    std::vector<std::string> vector1 = dict.at(string1);
    std::vector<std::string> vector2 = dict.at(string2);

    for (unsigned i = 0; i < std::max(vector1.size(), vector2.size()); i++){
