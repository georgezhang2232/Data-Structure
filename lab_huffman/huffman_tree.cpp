...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (singleQueue.size() + mergeQueue.size() != 1){
       ...
       Frequency sumfreq = templeft->freq.getFrequency() + tempright->freq.getFrequency();
       TreeNode* parentnode = new TreeNode(sumfreq);
       parentnode->left = templeft;
   ...
   HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     if (!bfile.hasBits()){
       ...
       if (bfile.getNextBit()){
         Frequency leafchar = Frequency(bfile.getNextByte(), 0);
         TreeNode* leaf = new TreeNode(leafchar);
         return leaf;
       }
       else {
