...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     pair<K, V>* Insert = new pair<K, V>(key, value);
     table[index] = Insert;
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
     unsigned int sizeNew = findPrime(size * 2);
     pair<K, V>** tableNew = new pair<K, V>*[sizeNew];
     delete[] should_probe;
     should_probe = new bool[sizeNew];
     for (unsigned i = 0; i < sizeNew; i++) {
       tableNew[i] = NULL;
       should_probe[i] = false;
     }
     ...
     for (unsigned i = 0; i < size; i++) {
       if (table[i]) {
         unsigned int index = hash(table[i]->first, sizeNew);
         while (tableNew[index] != NULL){
           index = (index + 1)% sizeNew;
         }
         tableNew[index] = table[i];
         should_probe[index] = true;
       }
     }
     delete[] table;
