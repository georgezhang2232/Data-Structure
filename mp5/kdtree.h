...
class KDTree
{
  ...
  public:
    ...
    void quickSort(vector<Point<Dim>>& points, int start, int end, int currDim);

    int partition(vector<Point<Dim>>& points, int start, int end, int currDim);

    KDTreeNode* KDTreehelper(vector<Point<Dim>>& newPoints, int start, int end, int Dimcount);
    /**
    ...
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* subtree, int currDim) const;
